...
前端默认请求 `https://api-sts2.urarawin.com/votes`；如需临时覆盖，可配置 `PUBLIC_VOTE_API_BASE`。

补充说明：

- 前端点击节流：连续点击会合并请求（防抖 240ms + 最小发送间隔 800ms），减少重复请求。
- 服务端限流：同一人对同一对象同一动作在 `THROTTLE_SECONDS` 窗口内会返回 429（默认 8 秒）。
- 本地开发代理：可在 `.env` 配置 `VOTE_API_PROXY_TARGET=https://api-sts2.urarawin.com`，让 `http://localhost:4321/api/*` 转发到目标地址。
