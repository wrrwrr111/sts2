...
async function renderWithSkin(dir, skelName, atlasName, skinName, outPath) {
  ...
  const idleNames = ["idle_loop", "idle"];
  let found = false;
  ...
  if (!isFinite(x1)) { console.log("  SKIP " + skinName + ": no bounds"); return; }
...
// Bowlbug: skins are default, cocoon, goop, rock, web
// Game names: Egg, Nectar, Rock, Silk
// Mapping: Egg=cocoon, Nectar=goop, Rock=rock, Silk=web
console.log("=== BOWLBUG ===");
await renderWithSkin(base + "/bowlbug", "bowlbug", "bowlbug", "cocoon", out + "/bowlbug_egg.png");
await renderWithSkin(base + "/bowlbug", "bowlbug", "bowlbug", "goop", out + "/bowlbug_nectar.png");
await renderWithSkin(base + "/bowlbug", "bowlbug", "bowlbug", "rock", out + "/bowlbug_rock.png");
await renderWithSkin(base + "/bowlbug", "bowlbug", "bowlbug", "web", out + "/bowlbug_silk.png");

// Cubex: skins are default, circleeye, diamondeye, moss1, moss2, moss3, squareeye
console.log("\n=== CUBEX ===");
await renderWithSkin(base + "/cubex_construct", "cubex_construct", "cubex_construct", "circleeye", out + "/cubex_construct.png");
await renderWithSkin(base + "/cubex_construct", "cubex_construct", "cubex_construct", "diamondeye", out + "/cubex_construct_diamond.png");
await renderWithSkin(base + "/cubex_construct", "cubex_construct", "cubex_construct", "squareeye", out + "/cubex_construct_square.png");

// Devoted sculptor: only "default" skin
console.log("\n=== DEVOTED SCULPTOR ===");
await renderWithSkin(base + "/devoted_sculptor", "devoted_scultpor", "devoted_sculptor", "default", out + "/devoted_sculptor.png");
